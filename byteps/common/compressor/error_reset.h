...
#ifndef BYTEPS_COMPRESSOR_ERROR_RESET_H
#define BYTEPS_COMPRESSOR_ERROR_RESET_H
 ...
 *
 * \par
 * Add error reset behavior to any compressor at run-time via decorator
 ...
 * \sa Compressor
 */
class ErrorReset : public Compressor {
 public:
  // error buffer should be cleared to zeros at the beginning.
  ErrorReset(size_t size, DataType dtype, std::unique_ptr<Compressor> cptr)
      : Compressor(size, dtype),
        _error(new byte_t[size]()),
        _prev_error(new byte_t[size]()),
        _cpu_reducer(new CpuReducer(nullptr)),
        _cptr(std::move(cptr)) {}
  virtual ~ErrorReset() = default;
 ...
 protected:
  ...
  std::unique_ptr<byte_t[]> _prev_error;
...
#endif  // BYTEPS_COMPRESSOR_ERROR_RESET_H